...
function tgs_shortcode_kurs_detail() {
    ...
    // Festes Template: strukturierte Textfelder + optionales Kursbild
    $kurz  = trim( (string) get_post_meta( $post_id, '_tgs_kurs_kurz', true ) );
    $ueber = trim( (string) get_post_meta( $post_id, '_tgs_kurs_ueber', true ) );
    $bild  = has_post_thumbnail( $post_id ) ? get_the_post_thumbnail_url( $post_id, 'large' ) : '';
    $hl_items = array();
    foreach ( preg_split( '/\r\n|\r|\n/', (string) get_post_meta( $post_id, '_tgs_kurs_highlights', true ) ) as $l ) {
        $l = trim( $l ); if ( $l !== '' ) $hl_items[] = $l;
    }
    $cap = function_exists( 'tgs_kurs_capacity' ) ? tgs_kurs_capacity( $post_id ) : null;
    ...
    <div class="tgs-kd-header">
        <div class="tgs-kd-header-l">
            ...
            <div class="tgs-kd-meta-row">
                ...
                <span>📍 <strong><?php echo tgs_ort_html( $ort, ! empty( $termin['ort_id'] ) ? $termin['ort_id'] : 0 ); ?></strong></span>
    ...
    <div class="tgs-kd-body">
        <div class="tgs-kd-content">
            ...
            <?php if ( $hl_items ) : ?>
            <div class="tgs-kd-highlights">
                <div class="tgs-kd-hl-title">Das erwartet dich</div>
                <ul><?php foreach ( $hl_items as $it ) echo '<li>' . esc_html( $it ) . '</li>'; ?></ul>
            </div>
            <?php endif; ?>
        ...
        <div class="tgs-kd-sidebar">
            <div class="tgs-kd-info-box">
                ...
                <?php if ( $ort ) : ?><div class="tgs-kd-info-row"><strong>Ort</strong><span><?php echo tgs_ort_html( $ort, ! empty( $termin['ort_id'] ) ? $termin['ort_id'] : 0 ); ?></span></div><?php endif; ?>
                ...
                if ( ! empty( $termin['saisonal'] ) ) {
                    ...
                    $lbl        = $ist_winter ? '☀️ Im Sommer' : '❄️ Im Winter';
                    if ( ! $ist_winter && ! empty( $termin['winter']['pause'] ) ) {
                        $wert = 'pausiert – kein Training';
                    } else {
                        $zt   = tgs_zeit_display( $andere['zeit'], $andere['ende'] );
                        $ol   = tgs_ort_html( $andere['ort'], isset( $andere['ort_id'] ) ? $andere['ort_id'] : 0 );
                        $wert = trim( esc_html( $zt ) . ( $zt && $andere['ort'] ? ' · ' : '' ) . $ol );
                    }
                    if ( $wert ) {
                        echo '<div class="tgs-kd-info-season">' . $lbl . ': <span>' . $wert . '</span></div>';
                    }
                }
                ?>
            </div>
...
function tgs_shortcode_navigation() {
    ...
    $html  = '<nav class="tgs-main-nav">';
    $html .= '<input type="checkbox" id="tgs-nav-toggle" class="tgs-nav-toggle" aria-label="Menü ein-/ausblenden">';
    $html .= '<label for="tgs-nav-toggle" class="tgs-nav-burger">☰ <span>Menü</span></label>';
    $html .= '<ul class="tgs-nav-list">';
